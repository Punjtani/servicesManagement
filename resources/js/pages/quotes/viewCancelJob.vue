        ...
        <div class="dashboard_content_area" v-if="!loading">
            <div class="row align-items-center mb-4" >
                ...
                <div class="col-md-1 d-flex justify-content-end">
                    <p @click="$router.go(-1)" class="h3 mb-2 cursor-pointer" v-bind:class="{ 'disable-forward': !canGoForward }"><b-icon icon="arrow-left-circle-fill"></b-icon></p>
                </div>
                ...
                <table class="table">
                    <thead>
                            <tr>
                                <td>Service</td>
                                <td >Requested For</td>
                                <td class="text-md-center_no">Requested for</td>
                                <td class="text-md-center_no" >Scheduled Date</td>
                                <td class="text-md-center_no">Scheduled Time</td>
                                <td class="text-md-center_no">Assigned To</td>

                            </tr>
                    </thead>
                    <tbody v-if="item.service" v-for="(item, index) in job.requested_job_service" :key="item.service_id" class="tbody_new">
                        <tr class="colapsible_tr" aria-expanded="false" aria-controls="panelsStayOpen-collapseThree">
                            ...
                            <td class="text-md-center_no" data-title = "Requested For"><strong>{{item.requested_for | formatDate}}</strong></td>
                            <td class="text-md-center_no" data-title = "Scheduled Date"><strong>{{item.scheduled_date | formatDate}}</strong></td>
                            <td class="text-md-center_no" data-title = "Scheduled Time" v-if="item.anytime == null"></td>
                            <td class="text-md-center_no" data-title = "Scheduled Time" v-else><strong>{{item.anytime == 1? "Any Time":item.scheduled_time | formatTime}}</strong></td>
                            <td class="text-md-center_no" data-title = "Assigned To"><strong>{{item.assigned_name}}</strong></td>
                        </tr>
                                                        ...
                                                        <table class="table">
                                                            <tbody>
                                                                <tr>
                                                                    ...
                                                                    <td data-title="Base Price" class="width_fifteen text-center">{{item1.base_price}}</td>
                                ...
                                <table class="table mg_0">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="width_fifteen" data-title="Total" >${{this.totalamount}}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </td>
        ...
        methods: {
            ...
            axios.get('/api/job/' + id + "/edit")
                .then((response) => {
                    ...
                    this.apartment_type = this.appartmentTypes.find(apartment => (apartment.id == this.job.apartment_type_id));
