...
export default {
    ...
    methods: {
        service_types_submit() {
            ...
            axios.put('/api/service-type/'+this.id, this.service_type_form)
                .then(response => {
                   ...
                   {
                    $("#serviceTypesEdit").modal('hide');
                    this.$emit('closeEvent',"Service Type has been updated successfully")
                   }
        ...
        getRecord(id) {
            this.loading = true;
            axios.get('/api/service-type/' + id + '/edit')
                .then((response) => {
                    this.service_type_form.category = response.data.service_type.category;
                    this.service_type_form.color = response.data.service_type.color;
                    this.service_type_form.payroll_type = response.data.service_type.payroll_type;
                    this.service_type_form.is_taxable = response.data.service_type.is_taxable;
                });
        },
        resetForm(){
