<template>
 ...
 <h4 class="modal-title"> Paint Grade</h4>
 <button type="button" class="btn-close" v-on:click="resetForm" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<form autocomplete="off" @submit.prevent="paint_grade_submit" method="post">
                        ...
                        <div class="form_box">
                            <div class="form-group">
                                ...
                                <div class="invalid-feedback" v-if="submit">
                                    <span v-if="!$v.paint_grade_form.name.required">Paint Color is Required</span>
                                    <span v-if="!$v.paint_grade_form.name.minLength">Paint Color length must be 2 Characters</span>
                                    <span v-if="!$v.paint_grade_form.name.maxLength">Paint Color length must not be greater then 20 Characters</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        paint_grade_submit() {
            ...
            axios.post('/api/paint-grade', this.paint_grade_form)
                .then(response => {
                    ...
                    this.$forceUpdate();
                    $("#PaintGradeAdd").modal('hide');
                    this.$emit('closeEvent',"Paint Grade  has been added successfully")
                })
                .catch(err => {
                    this.$v.paint_grade_form.$reset();
                    this.paint_grade_form = {
