<template>
 ...
 <h4 class="modal-title"> Paint Color</h4>
 <button type="button" class="btn-close" v-on:click="resetForm" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<form autocomplete="off" @submit.prevent="paint_color_submit" method="post">
...
export default {
    ...
    methods: {
        paint_color_submit() {
            ...
            axios.post('/api/paint-color', this.paint_color_form)
                .then(response => {
                    ...
                    $("#PaintColorAdd").modal('hide');
                    this.$emit('closeEvent',"Paint color  has been added successfully")
