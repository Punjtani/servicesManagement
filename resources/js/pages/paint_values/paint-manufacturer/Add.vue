<template>
 ...
 <h4 class="modal-title"> Paint Manufacturer</h4>
 <button type="button" class="btn-close" v-on:click="resetForm" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<form autocomplete="off" @submit.prevent="paint_manufacturer_submit" method="post">
                        ...
                        <div class="form_box">
                            <div class="form-group">
                                ...
                                <div class="invalid-feedback" v-if="submit">
                                    <span v-if="!$v.paint_manufacturer_form.name.required">Paint Color is Required</span>
                                    <span v-if="!$v.paint_manufacturer_form.name.minLength">Paint Color length must be 2 Characters</span>
                                    <span v-if="!$v.paint_manufacturer_form.name.maxLength">Paint Color length must not be greater then 20 Characters</span>
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        paint_manufacturer_submit() {
            ...
            axios.post('/api/paint-manufacturer', this.paint_manufacturer_form)
                .then(response => {
                    ...
                    this.$forceUpdate();
                    $("#PaintManufacturerAdd").modal('hide');
                    this.$emit('closeEvent',"Paint Manufacturer  has been added successfully")
                })
                .catch(err => {
                    this.$v.paint_manufacturer_form.$reset();
                    this.paint_manufacturer_form = {
