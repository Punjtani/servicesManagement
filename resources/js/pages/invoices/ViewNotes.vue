                    ...
                    <div class="modal-body" >
                        <div class="form_box" v-if="(before_attachments.length > 0||after_attachments.length > 0 || notes) ">
                            <div v-if="notes && (role == roleData.super_admin || role == roleData.admin)" class="form-group" >
                                <label v-if="role==roleData.client">Pictures </label>
                                <label v-else>Notes </label>
                                <textarea disabled class="form-control" placeholder="Job Notes" v-model="notes"></textarea>
                            </div>
                            <div v-if="before_attachments.length > 0" class="row align-items-center no-gutters attachments_box width_50">
                                <label>Before Attachments</label>
                                <br>
                                <div class="col-12 col-md-12 cols_5" v-for="(item1, index1) in before_attachments">                            
                                        <div class="img__box">
                                            <img @click="onImageClick(index1,before_attachments,'Before Attachments')" :src="'/'+item1.file_name">
                                        </div>                               
                                </div>                                
                            </div>
                            <div v-if="after_attachments.length > 0" class="row align-items-center no-gutters attachments_box width_50">
                                <label>After Attachments</label>
                                <br>
                                <div class="col-12 col-md-12 cols_5" v-for="(item1, index1) in after_attachments">                            
                                        <div class="img__box">
                        ...
                        <div v-if="(before_attachments.length <= 0) && (after_attachments.length <= 0) && (notes == '')" class="alert alert-danger">
    ...
    watch: {
        'notesData'(val) {
            if(val != null){
                ...
                this.before_attachments = val.service_job_progres_attatchment;
                this.after_attachments = val.service_job_progress_after_attatchment;
            }        
